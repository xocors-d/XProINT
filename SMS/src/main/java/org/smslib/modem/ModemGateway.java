...
//Eric20150322 - Fix the error on detecting ERROR respond
...
public class ModemGateway extends AGateway
{
		...
		for (int ml = 0; ml < (getATHandler().getStorageLocations().length() / 2); ml++)
		{
			if (getATHandler().switchStorageLocation(getATHandler().getStorageLocations().substring((ml * 2), (ml * 2) + 2)))
			{
				...
				while (true)
				{
					if (line == null) break;
					line = line.trim();
					//Eric20160404 if (line.length() <= 0 || line.equalsIgnoreCase("OK")) break;
					if (line.length() <= 0 || line.equalsIgnoreCase("OK")) {//Eric20160404
					//Eric20160404 Start
						while (true)
						{
							line = reader.readLine();
					...
					//Eric20160404 End
					//Eric20150322 if (line.length() <= 0 || line.equalsIgnoreCase("ERROR")) break;
					if (line.length() <= 0 || line.toUpperCase().indexOf("ERROR")!=-1) break;//Eric20150322
					i = line.indexOf(':');
					j = line.indexOf(',');
					memIndex = 0;
					try
					...
					/*20150918 Start*/
						...
						Logger.getInstance().logInfo(line+"(substring from "+(i+1)+" by "+j+")", e, getGatewayId());
						break;//Skip handling.....
					}
					/*20150918 End*/
					...
					try
					{
						...
						if (pdu instanceof SmsDeliveryPdu)
						{
							...
							Logger.getInstance().logInfo(pdu.toString(), null, getGatewayId());//Show received message.
							InboundMessage msg;
	...
	public void sendCustATCommand(String atCommand) throws GatewayException, TimeoutException, IOException, InterruptedException
	{
		synchronized (getDriver().getSYNCCommander())
		{
			getATHandler().sendCustATCommand(atCommand);
		}
	}
