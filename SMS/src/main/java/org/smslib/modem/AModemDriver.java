	...
	protected void connect() throws TimeoutException, GatewayException, IOException, InterruptedException
	{
		...
		synchronized (getSYNCCommander())
		{
			try
			{
				...
				//setAsyncNotifier(null);//20160425 remove Asyn Message Processor
				setAsyncMessageProcessor(new AsyncMessageProcessor());
				//setAsyncMessageProcessor(null);//20160425 remove Asyn Message Processor
				clearBuffer();
		...
		public void run()
		{
			while (isConnected())
			{
				try
				{
					...
					{
						//Read unread message only getGateway().readMessages(this.msgList, MessageClasses.ALL);
						getGateway().readMessages(this.msgList, MessageClasses.UNREAD);//Read unread message only 20160425
				...
				{
					e.printStackTrace();//Add to detect notification not send issue.
					if (!isConnected()) break;
				}
				catch (GatewayException e)
				{
